...
for i in wb[sheet].values:
    if i[m] == x[day] and i[e] in work_lst and (i[h] < 0.7 or i[g] == "线上（老师迟到15min）"):
        ...
        with open(f".\\error\\{week}异常情况登记表.csv", 'a', encoding='utf-8_sig', newline='') as fp:
            ...
            if i[h] < 0.6:
                if sheet == "二教":
                    writer.writerow(["", i[a], i[b], i[c], i[d], y[week], x[day], i[e], i[f], i[g], i[f].split("楼")[1], "", ""])
                if sheet == "天山堂":
                    writer.writerow(["", i[a], i[b], i[c], i[d], y[week], x[day], i[e], i[f], i[g], i[f], "", ""])
            else:
